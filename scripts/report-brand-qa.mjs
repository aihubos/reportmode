...
for (const file of htmlFiles(path.join(root, "reports"))) {
  ...
  const checks = [
    ...
    [/class="report-hub-logo-mark-shimmer" aria-hidden="true"/.test(html), "A 마크 반사광 레이어가 없음"],
    [/class="report-hub-logo-image"[^>]*src="[^"']*\/assets\/report-hub-logo\.png\?v=20260812-report-hub-logo2"[^>]*alt="Report Hub"/.test(html), "새 Report Hub 이미지 로고가 없음"],
...
for (const relative of ["index.html", "archive/index.html", "archive/upload.html"]) {
  ...
  if (!/favicon\.svg\?v=20260812-report-hub-logo2/.test(html)) failures.push(`${relative}: 최신 RH 파비콘이 없음`);
}
...
if (!archive.includes('class="archive-builders-card"') || !archive.includes('src="../assets/ai-builders-lab-logo.png"')) failures.push("archive/index.html: AI Builders Lab 당근모임 카드가 없음");
...
for (const relative of ["archive/index.html", "archive/upload.html"]) {
  ...
  if (!/class="report-hub-logo-mark-shimmer" aria-hidden="true"/.test(html)) failures.push(`${relative}: A 마크 반사광 레이어가 없음`);
  if (!/class="report-hub-logo-image"[^>]*src="[^"']*\/assets\/report-hub-logo\.png\?v=20260812-report-hub-logo2"[^>]*alt="Report Hub"/.test(html)) failures.push(`${relative}: 새 Report Hub 이미지 로고가 없음`);
...
if (!fs.existsSync(path.join(root, "assets", "report-hub-logo-mark.png"))) {
  failures.push("assets/report-hub-logo-mark.png: A 마크 반사광 자산이 없음");
}
