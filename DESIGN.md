...
- 메인 도서관 상단에서 네이버 블로그 카드 왼쪽에 둔다. 두 링크 묶음은 상단 우측에 정렬한다.
...
- 모바일에서는 두 카드를 함께 축소해 가로 넘침을 막는다.

### Hermes and LLM Wiki guidebook card

- 메인 도서관 상단의 기존 바로가기 카드 묶음에 한 번만 표시한다.
- 데스크톱은 기존 카드와 같은 142×48px, 모바일은 38×38px 크기를 사용한다.
- 데스크톱에는 `[가이드북] Hermes, LLM Wiki 설치`를 두 줄 이내로 표시하고, 모바일에서는 책 모양 선형 아이콘만 표시한다.
- 흰색 표면, `--rh-border` 경계선, `--rh-primary` 아이콘과 `--rh-ink` 글자를 사용한다.
- 새 창에서 Figma 교재를 열고 `noopener noreferrer`를 적용한다.
