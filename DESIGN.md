...
- 카드에는 `당근모임` 텍스트를 동일한 상단 카드 글꼴로 표시하고, 전체를 누르면 같은 모임 초대 링크를 새 창으로 연다.
- 카드 표면은 `--rh-paper`, 테두리는 `--rh-daangn`을 사용한다.
- 상단 당근·AI Builders Lab·네이버 카드 묶음은 우측에 정렬한다. 모바일에서는 방문자 수를 숨기고 세 카드를 축소해 함께 표시한다.

### AI Builders Lab meeting card

- 메인 도서관 상단에서 당근모임과 네이버 블로그 카드 사이에 둔다. 세 링크 묶음은 상단 우측에 정렬한다.
...
- 모바일에서는 세 카드를 함께 축소해 가로 넘침을 막는다.
