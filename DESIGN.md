...
- 메인 도서관 상단에서 네이버 블로그 카드 바로 왼쪽에 배치한다.
- 제공된 당근 로고 이미지만 데스크톱 68×40px, 모바일 60×34px으로 표시하고, 카드 전체를 누르면 같은 모임 초대 링크를 새 창으로 연다.
- 카드 표면은 `--rh-paper`, 테두리는 `--rh-daangn`을 사용한다. 로고 밖에 `당근모임` 문구를 따로 표시하지 않는다.
- 모바일에서는 방문자 수를 숨기고, 네이버 카드는 N 심벌만 표시해 두 링크가 잘리지 않게 한다.

### AI Builders Lab meeting card

- 메인 도서관 상단에서 당근 로고와 네이버 블로그 카드 사이에 둔다. 데스크톱에서는 세 링크 묶음을 상단 중앙에 정렬한다.
- 제공된 AI Builders Lab 로고 전체를 사용하고, 카드 전체를 누르면 당근 모임 초대 링크를 새 창으로 연다.
- 흰색 광택이 천천히 한 번씩 지나가는 효과를 사용하되, `prefers-reduced-motion` 환경에서는 움직임을 멈춘다.
- 모바일에서는 중복되는 당근 로고 카드는 숨기고 AI Builders Lab 카드와 네이버 N 카드만 남겨 가로 넘침을 막는다.
