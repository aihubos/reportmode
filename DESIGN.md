...
- 실제 모임 QR 이미지를 28px 정사각형으로 표시하고, 카드 전체를 누르면 같은 모임 초대 링크를 새 창으로 연다.
- `당근모임`은 `--rh-daangn`으로 표시하고, 카드 표면은 `--rh-paper`, 테두리는 `--rh-daangn`을 사용한다.
- 모바일에서는 방문자 수를 숨기고, 네이버 카드는 N 심벌만 표시해 `당근모임` 이름과 QR이 잘리지 않게 한다.
