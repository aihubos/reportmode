(function () {
  ...
  function openDelete(card, item) {
    ...
    form.append(el("p", "request-board-helper", "삭제하면 글과 관리자 답글이 함께 사라집니다."), labeled("작성 비밀번호", secret));
    var action = controls(form, "삭제 확정");
    form.addEventListener("submit", function (event) { event.preventDefault(); if (secret.value.length < 4) { secret.setAttribute("aria-invalid", "true"); secret.focus(); action.notice.textContent = message("password_too_short"); return; } action.save.disabled = true; requestJson(API + "/" + encodeURIComponent(item.id), { method: "DELETE", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ password: secret.value }) }).then(load).catch(function (error) { action.notice.textContent = message(error.message); }).finally(function () { action.save.disabled = false; }); });
  }
  ...
  function render(items) {
    ...
    items.forEach(function (item) {
      ...
      var actions = el("div", "request-board-actions"); if (item.editable) { var edit = makeButton("수정", "request-board-action"), remove = makeButton("삭제", "request-board-action"); edit.addEventListener("click", function () { openEdit(card, item); }); remove.addEventListener("click", function () { openDelete(card, item); }); actions.append(edit, remove); } else actions.appendChild(el("span", "request-board-edited", "이전 글 · 읽기 전용")); var replyButton = makeButton(item.admin_reply ? "관리자 답글 수정" : "관리자 답글", "request-board-action is-admin"); replyButton.addEventListener("click", function () { openReply(card, item); }); actions.appendChild(replyButton); card.appendChild(actions); list.appendChild(card);
    });
  }
